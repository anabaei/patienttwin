...
interface AppointmentsActions {
  bookAppointment: (payload: BookingPayload) => Promise<{ appointment: Appointment; booking: Booking }>;
...
export const useAppointmentsStore = create<AppointmentsStore>()(
  persist(
    (set, get) => ({
      ...
      rescheduleAppointment: async (appointmentId: string, _newSlotId: string) => {
        set({ isLoading: true, error: null });
